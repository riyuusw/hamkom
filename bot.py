...
auto_game_train = "y"
auto_minigame = "y"
auto_upgrade_multitap = "y" # Jika auto_upgrade_multitap = Y, sesuaikan lv_upgrade_multitap
lv_upgrade_multitap = 9
auto_upgrade_energy = "y" # Jika auto_upgrade_energy = Y, sesuaikan lv_upgrade_energy
lv_upgrade_energy = 9
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
auto_upgrade_pph = "y" # Jika auto_upgrade_pph = Y, lv_upgrade_multitap
...
#=========================================================================== 
id_promo_game = [{
    'id':'bike','promo_id':'43e35910-c168-4634-ad4f-52fd764a843f'},{
    'id':'clone','promo_id':'fe693b26-b342-4159-8808-15e3ff7f8767'},{
    'id':'cube','promo_id':'b4170868-cef0-424f-8eb9-be0622e8e8e3'},{
    'id':'train','promo_id':'c4480ac7-e178-4973-8061-9ed5b2e17954'}
]
data_game = []
if auto_balap_sepeda == "y":
    data_game.append({'name':'hAMSTER bIKE','file':file_vc_balapan})
if auto_game_clone == "y":
    data_game.append({'name':'cLONE aRMY','file':file_vc_clone})
if auto_game_cube == "y":
    data_game.append({'name':'cHAIN cUBE','file':file_vc_cube})
if auto_game_train == "y":
    data_game.append({'name':'tRAIN mINER','file':file_vc_train})
#print(data_game)
#time.sleep(5000)
session = HTMLSession()
ua = UserAgent()
init(autoreset=True)
def warna_random():
    colors = [Fore.BLUE, Fore.CYAN, Fore.GREEN, Fore.LIGHTBLACK_EX, Fore.LIGHTBLUE_EX, Fore.LIGHTCYAN_EX, Fore.LIGHTGREEN_EX, Fore.LIGHTMAGENTA_EX, Fore.LIGHTWHITE_EX, Fore.LIGHTYELLOW_EX, Fore.MAGENTA, Fore.WHITE, Fore.YELLOW]
...
def exchange(token):
    ...
    response = session.post(url, headers=headers, data=data)
    return response
def klaim_playground(token,pocer):
...
def claim_daily_combo(token):
    ...
    if response.status_code == 200:
        ...
        if error_response.get('error_code') == 'DAILY_COMBO_DOUBLE_CLAIMED':
            print(warna_random() + f"\r[ kOMBO hARIAN	] : {warna_random()}Kombo Sudah pernah dikalim          ", flush=True)
        else:
...
def main():
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                promo_data = response.json()['states']
                game_una = {}
                for a in id_promo_game:
                    try:
                        posisi_array = next((i for i, d in enumerate(promo_data) if d.get('promoId') == a['promo_id']), -1)
                        gametk = a['id']
                        kunci = promo_data[posisi_array]
                        game_una[f"{gametk}_now"] = kunci['receiveKeysToday']
                    except:
                        game_una[f"{gametk}_now"] = 0
            ...
            if response.status_code == 200:
                ...
                try:
                    jumlah_kunci = clicker_data['totalKeys']
                except KeyError:
                    jumlah_kunci = 0
                ...
                print(Fore.LIGHTYELLOW_EX + f"\r[ kUNCI		] : Total: {jumlah_kunci}   | Bike: {game_una['bike_now']}   | Clone: {game_una['clone_now']}   | Cube: {game_una['cube_now']}   | Train: {game_una['train_now']}", flush=True)
                print(Fore.LIGHTGREEN_EX + f"\r~~~~~~~~~~~~~~~~~~~~~~~~~~~[{Fore.LIGHTWHITE_EX} iNFO aUTO {Fore.LIGHTGREEN_EX}]~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                print(Fore.LIGHTYELLOW_EX + f"\r[ aUTO uPGRADE	] : {warna_random()}aBSEN: {auto_absen}  |  {warna_random()}tASK: {auto_cek_task_list}  | {warna_random()}mORSE: {auto_morse}  | {warna_random()}kOMBO: {auto_klaim_kombo}  | {warna_random()}mINIgAME: {auto_minigame}", flush=True)
                print(Fore.LIGHTCYAN_EX + f"\r[ aUTO uPGRADE	] : {warna_random()}bIKE: {auto_balap_sepeda}  | {warna_random()}cLONE: {auto_game_clone}  | {warna_random()}cUBE: {auto_game_cube}  | {warna_random()}tRAIN: {auto_game_train}", flush=True)
                print(Fore.LIGHTYELLOW_EX + f"\r[ aUTO uPGRADE	] : {warna_random()}eNERGI: {auto_upgrade_energy} - lV.mAKS: {lv_upgrade_energy}  |  {warna_random()}tAP: {auto_upgrade_multitap} - lV.mAKS: {lv_upgrade_multitap}", flush=True)
                print(Fore.LIGHTCYAN_EX + f"\r[ aUTO uPGRADE	] : {warna_random()}pPH: {auto_upgrade_pph} - mAX: {separator(harga_maksimal)} - cOOLDOWN wAIT: {tunggu_cooldown}\n", flush=True)
                #time.sleep(100000) 
                ...
                #time.sleep(10000)
                print(warna_random()+ f"\r[ Tap Status	] : {warna_random()}Tapping ...", end="", flush=True)
                response = tap(token, clicker_data['maxTaps'], clicker_data['availableTaps'])
                if response.status_code == 200:
                    print(f"\r{warna_random()}[ Tap Status	] : {Fore.LIGHTGREEN_EX}Sukses...            ", flush=True)
                    ...
                    if response.status_code == 200:
                        ...
                        for boost in booster_info:
                            if boost['id'] == 'BoostFullAvailableTaps':
                                ...
                                if stock == -1:
                                    stock ="Habis"
                                    print(f"\r{warna_random()}[ bOOSTER	] : {warna_random()}Stok {stock} | {warna_random()}Cooldown {jamnya}{menitnya}{detik} Detik    ", flush=True)
                                else:
                                    print(Fore.LIGHTGREEN_EX + f"\r[ bOOSTER	] : Stok {stock} | {warna_random()}Cooldown {jamnya}{menitnya}{detik} Detik    ", flush=True)
                        if cooldown == 0:
                ...
                if auto_absen == 'y':
                    ...
                    if response.status_code == 200:
                        ...
                        if daily_response['isCompleted']:
                            ...
                            print(f"\r{warna_random()}[ aBSEN hARIAN	] : {warna_random()} Hari ke {daily_response['days']} | Selesai", flush=True)
                    else:
                ...
                if auto_morse == 'y':
                    ...
                    if token not in claimed_ciphers:
                        ...
                        try:
                            if response.status_code == 200:
                                ...
                                if error_info.get('error_code') == 'DAILY_CIPHER_DOUBLE_CLAIMED':
                                    print(warna_random() + f"\r[ kLAIM mORSE	] : {warna_random()}Pernah diklaim         ", flush=True)
                                else:
                        ...
                        print(warna_random() + f"\r[ kLAIM mORSE	] : {warna_random()}Morse sudah pernah di-klaim sebelumnya.", flush=True)
				# Main mini Game
                if auto_minigame == 'y':
                    ...
                    if token not in claimed_minigame:
                        ...
                        try:
                            if response.status_code == 200:
                                ...
                                if error_info.get('error_code') == 'DAILY_KEYS_MINI_GAME_DOUBLE_CLAIMED':
                                    print(warna_random() + f"\r[ mINI gAME	] : {warna_random()}Pernah diklaim            ", flush=True)
                                else:
                        ...
                        print(warna_random() + f"\r[ mINI gAME	] : {warna_random()}Sudah main MiniGame sebelumnya            ", flush=True)
				# Playground Game
                if len(data_game) != 0:
                    for game in data_game:
                        if game['name'] == 'hAMSTER bIKE':
                             sisa = game_una['bike_now']
                        if game['name'] == 'cLONE aRMY':
                             sisa = game_una['clone_now']
                        if game['name'] == 'cHAIN cUBE':
                             sisa = game_una['cube_now']
                        if game['name'] == 'tRAIN mINER':
                             sisa = game_una['train_now']
                        if sisa < 4:
                            x = 1
                            while sisa <= 4:
                                print(Fore.LIGHTBLUE_EX + f"\r[ {game['name']}	] : Bisa klaim : {sisa} kali", end="", flush=True)
                                with open(game['file'], "r") as myfile:
                                    pocer = myfile.readline().strip('\n')
                                    myfile.close()
                                print(Fore.LIGHTGREEN_EX + f"\r[ {game['name']}	] : Voucher: {pocer}             ",  flush=True)
                                acak=random.randint(2,4)
                                countdown(acak)
                                response = klaim_playground(token,pocer)
                                if response.status_code != 200:
                                    p_salah = response.json()['error_code']
                                    if p_salah == 'InvalidPromoCode':
                                        pocer = input(f"\r[ {game['name']}	] : Vc salah/pernah diklaim: ")
                                        response = klaim_playground(token,pocer)
                                    elif p_salah == 'MaxKeysReceived':
                                        print(Fore.LIGHTRED_EX + f"\r[ {game['name']}	] : Sudah maksimal 4x       ", end="", flush=True)
                                        break
                                    else:
                                        print(Fore.LIGHTRED_EX + f"\r[ {game['name']}	] : Error : {p_salah}", end="", flush=True)
                                        break
                                print(Fore.LIGHTGREEN_EX + f"\r[ {game['name']}	] : Klaim {x} sukses. Lanjut..       ", end="", flush=True)
                                with open(game['file'], "r") as myfile:
                                    rows = myfile.readlines()[1:]
                                with open(game['file'], "w") as myfile:
                                    for item in rows:
                                        myfile.write(item)
                                x += 1
                        print(warna_random() + f"\r[ {game['name']}	] : {warna_random()}Sudah Klaim 4x hari ini            ", flush=True)
                # kombo harian
                        ...
                        if purchased_combos is None:
                            ...
                            for combo in njaluk_kombo("item"):
                                if combo in purchased_combos:
                                    print(warna_random() + f"\r[ kOMBO hARIAN	] : {warna_random()}{combo} sudah dibeli.", flush=True)
                                elif combo == "none":
                                    continue
                                else:
                            ...
                            if purchased_combos == required_combos:
                                ...
                                print(Fore.LIGHTRED_EX + f"\r[ kOMBO hARIAN	] : Gagal. Kombo belum terbeli: {required_combos - purchased_combos}               " , flush=True)
                                combo_upgraded[init_data_raw] = False
                ...
                if auto_cek_task_list == 'y':
                    ...
                    if not cek_task_dict[token]:  # Cek status cek_task untuk token ini
                        ...
                        if response.status_code == 200:
                            ...
                            if all_completed:
                                print(warna_random() + f"\r[ LiST tASK	] : {warna_random()}Sudah diklaim\n", flush=True)
                            else:
                                for task in tasks:
                                    if not task['isCompleted']:
                                        ...
                                        if response.status_code == 200 and response.json()['task']['isCompleted']:
                                            print(warna_random() + f"\r[ LiST tASK	] : {warna_random()}Pernah Diklaim sebelumnya {task['id']}\n", flush=True)
                                        else:
...
if __name__ == "__main__":
    while True:
        try:
            ...
            print(Fore.LIGHTRED_EX + f"\nUnhandled exception: {str(e)}. Restarting the script...\n", flush=True)
